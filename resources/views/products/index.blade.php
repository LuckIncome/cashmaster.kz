                  ...
                  <table class="table table-bordered mb-0">
                     <thead>
                        <tr>
                           ...
                           <th>Драйвер</th>
                           <th>Цена для юридических лиц</th>
                           <th>Цена для физических лиц</th>
                           <th>Валюта</th>
                           <th>Контроль</th>
                           {{-- <th>Роль</th> --}}
                           {{-- <th>Верификация</th> --}}
                     ...
                     @foreach ($product as $item)
                        <tr>
                           ...
                           @if($item->file==NULL)<td>Драйвера нет</td>@else<td>{{ $item->file }}</td>@endif
                           <td>{{ $item->entityprice }}</td>
                           ...
                           <td>
                            ...
                            <form action="{{ route('products.destroy', $item->id) }}" method="POST" style ='display: inline-block;'>
                                @method('DELETE')
                                @csrf
                                <button type="submit" class="btn btn-danger btn-sm waves-effect" onclick="return confirm('Вы уверены, что хотите удалить товар?')">Удалить</button>
                             </form>
                            </td>
                        </tr>
                     @endforeach
