            ...
            <div class="card">
               <div class="card-body">
                    ...
                    <p class="text-muted m-b-15">
                        <a href="{{ route('seo.create') }}"><button class="btn btn-primary btn-sm waves-effect">Создать партнера</button></a> 
                    </p>
                  <table class="table table-bordered mb-0">
                     ...
                     @foreach ($seo as $item)
                        <tr>
                           ...
                           <td>
                            <a href="{{ route('seo.show', $item->id) }}"><button type="submit" class="btn btn-success btn-sm waves-effect">Показать</button></a>
                            <a href="{{ route('seo.edit', $item->id) }}"><button type="submit" class="btn btn-warning btn-sm waves-effect">Редактировать</button></a>
                            <form action="{{ route('seo.destroy', $item->id) }}" method="POST" style ='display: inline-block;'>
                                @method('DELETE')
                                @csrf
