                ...
                <form action="/order" method="post" id="store">
                    ...
                    <div class="of-row1">
                        ...
                        <div class="form-group">
                            <input type="text" class="form-control" name="userid" value="{{ Auth::user()->id }}"
                            required>
                            <input type="text" class="form-control" name="name_tov" value="@foreach($products as $product)@if(count($product) < 2){{$product['item']['name']}}@else{{$product['item']['name']}}@endif|@endforeach"
                            required>
                            <input type="text" class="form-control" name="amount" value="@foreach($products as $product)@if(count($product) < 2){{$product['qty']}}@else{{$product['qty']}}@endif|@endforeach"
                            required>
                        </div>
                        <div class="form-group">
                            <input type="text" class="form-control" name="address" value="{{ number_format(session()->get('cart')->totalPrice, 0, ',', '')* $currency[0]->coefficient }}"
                            required>
                        </div>
                        @else
                        <div class="form-group">
                        ...
                        <div class="form-group">
							...
							@else
							    ...
							    @foreach($products as $product)<input type="text" class="form-control" name="address" value="{{ $product['item']['individualprice']* $currency[0]->coefficient }}" required>@endforeach
							@endif
                        </div>
                        @endif
                    </div>
                    <div class="of-row2">
                        ...
                        <div>
                            ...
                            <select class="sss" name="payment">
                                ...
                                <option value="/qiwi-terminal">QIWI-кошелёк</option>
                            </select>
                        </div>
<style>
            ...
            width: 228px !important;
                ...
                <div class="of-row3">
                        <div class="of1">
                                ...
                                <h2>{{ number_format(session()->get('cart')->totalPrice * $currency[0]->coefficient, 0, ',', '') }}</h2>
