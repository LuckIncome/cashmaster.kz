	...
	public function qiwiterminal(Request $request){
		 ...
		 if(session()->has('cart'))
        {
			...
			$order_save = orders_user::create([
					...
					'city' => $request['city'],
					'address' => $request['address_user'],
					'totalprice' => (float)$cart->totalPrice * $currency[0]->coefficient,
					'status' => 1,
					'paymenttype' => $request['payment'],
					'shippingttype' => $request['post'],
				]);
			 ...
			 foreach($cart->items as $product) {
					 ...
					 order_products::create([
						...
						'name' => $product['item']['name'],
						'count' => $product['qty'],
						'price' => $product['individualprice']* $currency[0]->coefficient,
					]);   
             }
		}
  ...
  'order_total' => $order_save->totalprice,
	...
	function sendemail($request, $cart, $order_id){
		...
		$message = "Имя: ".$request['name']."\n";
				$message .= "Сумма заказа: ".(float)$cart->totalPrice * $currency[0]->coefficient." тг\n";
    ...
    public function paybox(Request $request){
        ...
        $request = [
            'pg_amount' => $cart->totalPrice * $currency[0]->coefficient, //общая сумма платежа
