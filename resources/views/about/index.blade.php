   ...
   <div class="container-fluid">
      ...
      <div class="row">
         <div class="col-12">
            <div class="card">
               <div class="card-body">
                    <h4 class="mt-0 header-title">О компании</h4>
                    <p class="text-muted m-b-30 font-14">О компании.</p>
                    <p class="text-muted m-b-15">
                        {{-- <a href="{{ route('about.create') }}"><button class="btn btn-primary btn-sm waves-effect">Создать пользователя</button></a>  --}}
                    </p>
                  <table class="table table-bordered mb-0">
                     ...
                     @foreach ($about as $item)
                        <tr>
                           <td>{{ $item->id }}</td>
                           <td>{{ $item->name }}</td>
                           <td style="width: 50%;">{!! $item->video !!}</td>
                           <td>{!! $item->description !!}</td>
                           {{-- <td><img src="{{ Storage::url($item->image) }}" alt="{{ $item->name }}" style="width:150px;"></td> --}}
                           <td>
                            <a href="{{ route('about.show', $item->id) }}"><button type="submit" class="btn btn-success btn-sm waves-effect">Показать</button></a>
                            <a href="{{ route('about.edit', $item->id) }}"><button type="submit" class="btn btn-warning btn-sm waves-effect">Редактировать</button></a>
