...
@section('content')
<style>
    table.table.table-bordered.mb-0 img {
        width: 100% !important;
    }
</style>
<!-- Start content -->
            ...
            <div class="card">
               <div class="card-body">
                    ...
                    <p class="text-muted m-b-15">
                        <a href="{{ route('articles.create') }}"><button class="btn btn-primary btn-sm waves-effect">Создать информацию</button></a> 
                    </p>
                  <table class="table table-bordered mb-0">
                     ...
                     @foreach ($article as $item)
                        <tr>
                           <td>{{ $item->id }}</td>
                           <td>{{ $item->name }}</td>
                           <td>{!! \Illuminate\Support\Str::words($item->description, 100,'....')  !!}</td>
                           {{-- <td><img src="{{ Storage::url($item->image) }}" alt="{{ $item->name }}" style="width:150px;"></td> --}}
                           <td>
                            <a href="{{ route('articles.show', $item->id) }}"><button type="submit" class="btn btn-success btn-sm waves-effect">Показать</button></a>
                            <a href="{{ route('articles.edit', $item->id) }}"><button type="submit" class="btn btn-warning btn-sm waves-effect">Редактировать</button></a>
